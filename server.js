...
// ── Agent: clients — initialized inside runAgent() so missing env vars at
//    module load time don't crash the process before the server starts ─────────
...
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:Arial,sans-serif">
  <tr>
    ...
    <td style="border-left:2px solid #1a1a1a;padding-left:16px;vertical-align:middle">
      <p style="margin:0 0 1px;font-size:13px;font-weight:800;color:#1a1a1a;font-family:Arial,sans-serif"><strong>LUCY GALLIFORD</strong></p>
      ...
      <p style="margin:0;font-size:11px;font-family:Arial,sans-serif">
        <a href="https://drive.google.com/file/d/1boH2eQqsTWxKKJosL4pQrGBaYwWlWvTx/view" style="color:#1a1a1a;text-decoration:none">&#128206; View my Media Kit</a>
      </p>
    </td>
  </tr>
</table>`;
...
const PLAIN_TEXT_SIGNATURE = [
  ...
  'Media Kit: https://drive.google.com/file/d/1boH2eQqsTWxKKJosL4pQrGBaYwWlWvTx/view',
].join('\n');
...
async function detectReplies() {
  ...
  for (const p of prospects) {
    try {
      ...
      console.error(`  Reply check failed for ${p.company_name}: ${err.message}`);
...
Use this template exactly, substituting only the first name and brand name:
...
Output the email body only, nothing else.`
      ...
      console.log(`  Follow-up 1 -> ${p.company_name}`);
      summary.followUpsSent++;
      summary.followUpsSentList.push({ company: p.company_name, status: 'follow_up_1_sent' });
      await sleep(2000);
    } catch (err) {
      console.error(`  FU1 failed for ${p.company_name}: ${err.message}`);
  ...
  for (const p of prospects) {
    ...
    try {
      if (isSkipListed(p.company_name, skipList)) {
        console.log(`  ${p.company_name} — on skip list`);
        continue;
      }
      ...
      if (activeDuplicate) {
        console.log(`  ${p.company_name} — already in pipeline (${activeDuplicate.status})`);
        continue;
      }

      const domain = p.contact_email.split('@')[1];
      if (await hasGmailHistory(p.company_name, domain)) {
        console.log(`  ${p.company_name} — existing relationship, skipping`);
        continue;
      }
...
Rules: no exclamation marks, warm but professional tone, output body only.`,
      ...
      console.log(`  Outreach -> ${p.contact_name} at ${p.company_name}`);
      summary.emailsSent++;
      summary.outreachSentList.push({ company: p.company_name, contact: p.contact_name, email: p.contact_email });
      await sleep(2000);
    } catch (err) {
      console.error(`  Outreach failed for ${p.company_name}: ${err.message}`);
...
Only suggest brands where a genuine collaboration would make sense for a cinematic travel filmmaker with 90k followers.`,
  ...
  for (const p of (prospects || []).slice(0, 5)) {
    try {
      ...
      console.log(`  Looking up ${p.contact_name} at ${p.company_name} (${p.company_domain})...`);
      const email = await findVerifiedEmail(p.contact_name, p.company_domain);
      if (!email) {
        console.log(`  No verified email for ${p.contact_name} at ${p.company_domain} — skipping`);
        continue;
      }
      ...
      console.error(`  Error processing ${p.company_name}: ${err.message}`);
...
async function runAgent() {
  // Initialize clients here so missing env vars at startup don't crash the process
  ...
  gmail     = google.gmail({ version: 'v1', auth: agentAuth });
  anthropic = new Anthropic({ apiKey: ANTHROPIC_API_KEY });

  _labelCache = null; // reset per-run
...
app.get('/gmail/unread', async (req, res) => {
  try {
    ...
    const listRes = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/messages?q=is:unread&maxResults=50', {
  ...
  try {
    ...
    const events = (calData.items || []).map(ev => {
      ...
      return {
        ...
        durationMins,
        type: 'event',
        needsPrepTask: durationMins >= 15 && !!(ev.summary || '').toLowerCase().match(/call|meet|interview|sync|chat/),
        prepTaskText: `Prep for: ${ev.summary}`,
      };
    });
    res.json({ events });
  } catch (err) { res.status(err.message === 'NOT_AUTHENTICATED' ? 401 : 500).json({ error: err.message }); }
...
// POST /run-agent
// Called by cron-job.org at 7am Europe/London.
// Set CRON_SECRET on Render; configure cron-job.org to send:
...
// GET /agent/summary — today's daily-summary.json (for the dashboard)
