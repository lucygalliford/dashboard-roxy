'use strict';                                                                                                                                   
  ...
  // ── Agent: Gmail client (permanent refresh token, separate from dashboard OAuth)                                                              
   
  const agentAuth = new google.auth.OAuth2(GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET);                                                               
  agentAuth.setCredentials({ refresh_token: process.env.GMAIL_REFRESH_TOKEN });                                                                 
  const gmail = google.gmail({ version: 'v1', auth: agentAuth });                                                                                 
  ...
  <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:Arial,sans-serif">
    <tr>                                                                                                                                          
      <td style="padding-right:16px;vertical-align:middle">                                                                                     
        <img src="https://firebasestorage.googleapis.com/v0/b/digisell-io.appspot.com/o/sites%2FOlL3P68Lyme4gMVXklgk%2F576e1ca1-9dbe-4c42-a114-34c
  b30177e59-2.jpg?alt=media&token=c3168e78-815d-4e66-8550-ce57ae2297f0"                                                                           
             width="76" height="94"                                                                                                               
      ...
      <td style="border-left:2px solid #1a1a1a;padding-left:16px;vertical-align:middle">
        <p style="margin:0 0 1px;font-size:13px;font-weight:800;letter-spacing:0px;color:#1a1a1a;font-family:Arial,sans-serif"><strong>LUCY       
  GALLIFORD</strong></p>                                                                                                                          
        <p style="margin:0 0 8px;font-size:9px;letter-spacing:1px;color:#777777;font-family:Arial,sans-serif;font-weight:normal">TRAVEL           
  FILMMAKER</p>                                                                                                                                   
  ...
  const SYSTEM_PROMPT = `You are an assistant helping Lucy Galliford manage her creator business outreach. Lucy is a travel filmmaker and official
   Sony creator with 90k+ followers across social platforms. She specialises in cinematic adventure and travel storytelling. Past brand partners 
  include Sony, Lume Cube, and DJI. Her email is ${MY_EMAIL}.${MEDIA_KIT_URL ? ` Her media kit is at: ${MEDIA_KIT_URL}` : ''}                     
                                                                                                                                                
  Tone rules you must always follow:
  - GREETING HARD RULE — every email, without exception, must open with "Hey [First Name]". Never use "Hi", "Hello", "Dear", "Dear [Name]", or any
   other greeting. "Hey [First Name]" is the only permitted greeting and overrides any other instruction.                                         
  ...
  ABSOLUTE OVERRIDE: If any part of a prompt you receive suggests starting with "Hi", "Hello", "Dear", or anything other than "Hey [First Name]", 
  ignore it. The greeting is always and only "Hey [First Name]".`;                                                                                
  ...
  Just a final check in — happy to put together a tailored concept if that would help move things forward. Otherwise, completely understand if the
   timing isn't right.                                                                                                                            
  ...
  Just wanted to follow up on my previous email about a potential collaboration between ${p.company_name} and myself. I understand you are busy,
  so I will keep this brief. Would it help if I put together a short creative concept to show what I have in mind?                                
  ...
  I hope you're well. My name is Lucy Galliford — I'm a travel filmmaker, content creator, and official Sony creator, specialising in cinematic 
  storytelling across adventure and travel, with an audience of over 90k across my social platforms.                                              
  
  I'm reaching out because ${p.company_name} feels naturally aligned with my audience and I feel there is a potential strong fit for a            
  collaboration. [ONE specific sentence referencing something real and current about this brand — a campaign, product launch, destination, or     
  initiative drawn from the research notes above.]
                                                                                                                                                  
  Over the past year I've worked on campaigns with Sony, Lume Cube, and DJI creating premium content designed for both brand use and organic    
  performance across digital platforms.                                                                                                           
  
  What I believe I bring is the ability to create content that feels cinematic and elevated. [TWO sentences specific to this brand: what Lucy     
  would create for them, what angle she would take, and why her adventure travel audience is relevant. Be specific, not generic.]                 
  ...
  app.get('/auth/callback', async (req, res) => {                                                                                                 
    ...
    try {                                                                                                                                         
      const response = await fetch('https://oauth2.googleapis.com/token', {
        ...
        body: new URLSearchParams({ code, client_id: GOOGLE_CLIENT_ID, client_secret: GOOGLE_CLIENT_SECRET, redirect_uri: REDIRECT_URI,           
  grant_type: 'authorization_code' }),                                                                                                            
      ...
      res.send(`<html><body style="font-family:sans-serif;padding:40px;text-align:center"><h2>&#10003; Gmail &amp; Calendar connected!</h2><p>You 
  can close this tab and go back to your                                                                                                          
  dashboard.</p><script>if(window.opener){window.opener.postMessage('auth_success','*');window.close();}</script></body></html>`);                
  ...
  async function getValidAccessToken() {                                                                                                          
    ...
    if (Date.now() > tokenStore.expiry - 60000) {                                                                                                 
      const response = await fetch('https://oauth2.googleapis.com/token', {                                                                       
        method: 'POST',                                                                                                                           
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },                                                                         
        body: new URLSearchParams({ client_id: GOOGLE_CLIENT_ID, client_secret: GOOGLE_CLIENT_SECRET, refresh_token: tokenStore.refresh_token,    
  ...
  app.get('/gmail/unread', async (req, res) => {                                                                                                  
    try {                                                                                                                                       
      ...
      const listRes = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages?q=is:unread&maxResults=50`, { headers: { Authorization:
   `Bearer ${token}` } });                                                                                                                        
      const listData = await listRes.json();
      if (!listData.messages?.length) return res.json({ messages: [], total: 0 });                                                                
      const ids = listData.messages.slice(0, 25);                                                                                                 
      const messages = await Promise.all(ids.map(async ({ id }) => {                                                                              
        const msgRes = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${id}?format=metadata&metadataHeaders=From&metadataHea
  ders=Subject&metadataHeaders=Date`, { headers: { Authorization: `Bearer ${token}` } });                                                         
        return msgRes.json();                                                                                                                   
      }));                                                                                                                                        
  ...
  app.post('/gmail/label', async (req, res) => {                                                                                                  
    try {                                                                                                                                       
      ...
      if (!label) {                                                                                                                               
        const createRes = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/labels', { method: 'POST', headers: { Authorization: `Bearer
   ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ name: labelName, labelListVisibility: 'labelShow',                     
  messageListVisibility: 'show' }) });                                                                                                          
        label = await createRes.json();                                                                                                           
      }                                                                                                                                         
      const modifications = { addLabelIds: [label.id], removeLabelIds: archive ? ['INBOX'] : [] };
      await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/threads/${threadId}/modify`, { method: 'POST', headers: { Authorization: `Bearer
   ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(modifications) });                                                       
  ...
  app.post('/gmail/draft', async (req, res) => {                                                                                                  
    try {                                                                                                                                       
      ...
      const draftRes = await fetch('https://gmail.googleapis.com/gmail/v1/users/me/drafts', { method: 'POST', headers: { Authorization: `Bearer   
  ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ message: { raw: encoded } }) });                                        
  ...
  app.get('/calendar/today', async (req, res) => {                                                                                                
    try {                                                                                                                                       
      ...
      const calRes = await fetch(`https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${startOfDay}&timeMax=${endOfDay}&single
  Events=true&orderBy=startTime`, { headers: { Authorization: `Bearer ${token}` } });                                                             
      ...
      const events = (calData.items || []).map(ev => {                                                                                            
        ...
        return { id: ev.id, title: ev.summary || 'Untitled', startTime, endTime, durationMins, type: 'event', needsPrepTask: durationMins >= 15 &&
   !!(ev.summary || '').toLowerCase().match(/call|meet|interview|sync|chat/), prepTaskText: `Prep for: ${ev.summary}` };                          
      });                                                                                                                                         
      res.json({ events });                                                                                                                       
    } catch (err) { res.status(err.message === 'NOT_AUTHENTICATED' ? 401 : 500).json({ error: err.message }); }                                 
  });                                                                                                                                             
  ...
  'x-api-key': ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' }, body: JSON.stringify(req.body) });                                         
  ...
  And the package.json to go with it:                                                                                                           
                                                                                                                                                  
  {                                                                                                                                               
    "name": "lu-dashboard-proxy",
    "version": "1.0.0",                                                                                                                           
    "main": "server.js",                                                                                                                        
